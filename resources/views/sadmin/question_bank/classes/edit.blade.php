...
@section('content')
    <div data-aos="fade-up" data-aos-duration="1000" class="p-sm-30 p-15">
        ...
        <div class="bd-one bd-c-stroke bd-ra-10 bg-white p-25">
            <form action="{{ route('super-admin.question-bank.classes.update', $class->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" name="order" class="form-control" value="{{ $class->order }}">
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="form-label">{{ __('Status') }} <span class="text-danger">*</span></label>
                        <select name="status" class="form-select" required>
                            <option value="1" {{ $class->status == 1 ? 'selected' : '' }}>{{ __('Active') }}</option>
                            <option value="2" {{ $class->status == 2 ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                        </select>
                    </div>
                </div>
